...
yay -S gitflow-avh ttf-font-awesome otf-overpass arc-gtk-theme orchis-theme-git ttf-ms-fonts ttf-vista-fonts ttf-cascadia-code manjaro-pulse python-pywal xca unzip zip rar xarchiver bluez bluez-utils blueman gnome-disk-utility-git firefox python-pywalfox jdk11-openjdk scala sbt intellij-idea-ultimate-edition libreoffice-fresh libreoffice-fresh-es texlive-most texstudio ghostscript biber r rstudio-desktop-bin virtualbox redshift redshift-gtk-git insync flatpak playerctl pulseaudio-equalizer-ladspa pavucontrol vlc kodi pulseaudio-modules-bt-git paprefs kdenlive telegram-desktop keepassxc gimp inkscape rofi polybar polybar-spotify-git skypeforlinux-stable-bin gufw clamtk manjaro-printer hplip isw autofirma neofetch exa spicetify-cli spicetify-themes-git thunar thunar-archive-plugin steam-manjaro discord tumbler tumbler-extra-thumbnailers ripgrep-all alacritty jq pcsx2 simple-scan simplescreenrecorder dia chromium ranger nerd-fonts-cascadia-code
...
spicetify config current_theme $SPOTIFY_THEME
